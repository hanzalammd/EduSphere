...
export default function Classes({readOnly=false}:{readOnly?:boolean}){
 ...
 return <section className="page classes-page">
   ...
   {!selected?<div className="classes-overview card"><div className="section-heading"><div><h3>All Classes</h3><p>Select a class to open its details.</p></div><button className="secondary small" onClick={load} aria-label="Refresh classes"><RefreshCw size={15}/></button></div>{loading?<div className="empty">Loading classes…</div>:classes.length===0?<div className="empty-state-box"><School size={24}/><b>No classes yet</b><span>Create the first class to get started.</span>}</div>:<div className="class-cards clean-class-grid">{classes.map(c=><div key={c.id} className="class-card"><button className="class-card-open" onClick={()=>{setSelected(c);setTab('timetable');setDay('Monday')}}><span className="class-icon"><School size={19}/></span><span><b>{c.name}</b><small>{teacherName(c.class_teacher_id)}</small></span></button>{!readOnly&&<button className="class-delete-btn" title={`Delete ${c.name}`} aria-label={`Delete ${c.name}`} onClick={()=>deleteClass(c)} disabled={saving}><Trash2 size={16}/></button>}<button className="class-arrow-btn" aria-label={`Open ${c.name}`} onClick={()=>{setSelected(c);setTab('timetable');setDay('Monday')}}>›</button></div>)}</div>}</div>
