...
export default function Attendance({teacher=false,admin=false}:{teacher?:boolean;admin?:boolean}){
 const [classes,setClasses]=useState<ClassRow[]>([]),[selectedClass,setSelectedClass]=useState(''),[people,setPeople]=useState<any[]>([]),[records,setRecords]=useState<Record<string,any>>({});
 ...
 const load=async()=>{setLoading(true);setError('');try{
   let p:any[]=[];
   if(teacher){ const {data:{user}}=await supabase.auth.getUser(); p=await rows('teachers','*','full_name',true); if(!admin && user) p=p.filter((x:any)=>x.profile_id===user.id); }
   ...
   const {data,error:e}=await supabase.from(attendanceTable).select('*').gte('attendance_date',start).lte('attendance_date',end); if(e)throw e;
 ...
 const save=async()=>{setSaving(true);setError('');setMsg('');try{const {data:{user}}=await supabase.auth.getUser();if(!user)throw new Error('You are not signed in.');
   for(const p of people)for(const d of days){const k=`${p.id}|${dateKey(month,d)}`,r=records[k];if(!r||!r.status)continue;if(!admin&&records[k]?.created_at)continue;
     const payload:any={[idField]:p.id,attendance_date:dateKey(month,d),status:r.status,marked_by:user.id};
 ...
 return <section className="page attendance-page">
  ...
  <div className="attendance-summary">{!teacher&&className&&<b>{className.name}</b>}<span>{people.length} {teacher?'teachers':'students'}</span></div>
  ...
  <div className="attendance-footnote">Previous months remain saved with their exact dates. <b>Only Admin can edit a recorded entry.</b> Teachers cannot change an attendance record after it has been saved.</div>
 </section>
}
